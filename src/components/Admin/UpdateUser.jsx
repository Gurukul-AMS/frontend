...
export default function EditStudentProfile() {
    ...
    const [oneUser, thisUser] = useState();
    ...
    function sendInfo() {
        ...
        axios.post("http://localhost:5000/api/updateuser", querystring.stringify({whichUser: oneUser, firstName: firstName, lastName: lastName, email: email}), {
            headers: {
    ...
    function userOptions() {
        if(users) {
            return (users.map((option) => <option key={option.username} value={option._id}>
            {option.username}
          </option>));
        } else {
            return "Oops!";
